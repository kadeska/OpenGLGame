...
private:
    ...
    void loadModel(std::string const& path)
    {
		...
		int result = FileManager::initModelDataFolder();
        if(result == -1)
        {
            std::cout << "ERROR::FILEMANAGER:: Could not create model data folder." << std::endl;
        ...
        if(result == 0)
        {
            std::cout << "INFO::FILEMANAGER:: Model data folder didn't exist, folder was created successfully. Loading model data from models. This may take a while..." << std::endl;
			loadModelFile(path);
		}
    ...
    void loadModelFile(std::string const& path)
    {
        ...
        modelName = path.substr(path.find_last_of('/') + 1, path.find_last_of('.') - path.find_last_of('/') - 1);
    ...
    Mesh processMesh(aiMesh* mesh, const aiScene* scene)
    {
        ...
        for (unsigned int i = 0; i < mesh->mNumVertices; i++)
        {
			...
			// save to file buffer
			FileManager::saveToBuffer(std::string("X:" + std::to_string(vertex.Position.x) + " Y:" + std::to_string(vertex.Position.y) + " Z:" + std::to_string(vertex.Position.z) + "\n"));
        ...
        for (unsigned int i = 0; i < mesh->mNumFaces; i++)
        {
            ...
            for (unsigned int j = 0; j < face.mNumIndices; j++) {
				...
				FileManager::saveToBuffer("I:" + std::to_string(face.mIndices[j]) + "\n");
            }
        }
        FileManager::saveToBuffer("\n::\n");  // separator
		//FileManager::saveBufferToFile("bufferdata");
		...
		for (const auto& texture : textures) {
			FileManager::saveToBuffer("T:" + texture.type + " P:" + texture.path + " ID:" + std::to_string(texture.id) + "\n");
		}

		FileManager::saveBufferToFile(modelName + "_mesh" + std::to_string(meshCount));
